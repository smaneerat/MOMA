 ...
 *   Updated : 10/02/2016
 *   This version is for ONLY use in the experimentation of mosquitoes'dispersal and age test depending on geographical discontinuity in urban area
 *   The simulation exemple in this code is applied to Hauz Rani village in Delhi (India) with 4 testing urban scenarios.
 *
...
global 
{
	 ...
	 ************************************/
	 bool bCheckActStat <- true parameter: "check activites?" category: 'Simulation Options';
	 bool bCheckMaxDisp <- true parameter: "check max dispersal?"  category: 'Simulation Options';
	 bool bCheckNbBiteOvipoPerHourInObj <- false parameter: "Check nb bites/ovipos per hous in each spatial obj?" category: 'Simulation Options';
	 bool bCheckFinalFuzzyScore <- false parameter: "show final fuzzy score?" category: 'Simulation Options';
	 bool bCheckStck <- false parameter: "check Aedes'stocks?"  category: 'Simulation Options';
	 bool bAutoSimStop <- false parameter: "auto stopping?" category: 'Simulation Options';
	 bool bDisprsShpNeed <- false parameter: "need dispersal shapefile?"  category: 'Simulation Options';
	 bool bUpdateStck <- false parameter: "update aquatic stocks?"  category: 'Simulation Options';
	 bool bNeedNewZone <- false parameter: "need new study zone shapefile?" category: 'Simulation Options';
	 bool bShowLimitArea <- false parameter: "show limit area for Aedes?" category: 'Simulation Options';
	 bool bTrackAedes <- false parameter: "track Aedes movement?" category: 'Simulation Options';
	 bool bTrackFuzzyProcess <- false parameter: "Track fuzzy process? " category: 'Simulation Options';
	 bool bAvgDenseBSMode <- true parameter: "use average density for BS?" category: 'Sim Options';
	...
	float BLOOD_USE_LAYEGG_RATE <- 0.03; // estimate 100 eggs / 3mg (full blood meal)
	 ...
	 float MIN_TEMP_LAYEGG <- 18.0;//°C
	 
	 //------- re-bite probability -----------
	 float MAX_STOCK_BLOOD <- 3.0;//maximum 3 mg/1 gono cycle
	 
	//-------- flight probability --------------
	float TARGET_MAXDIST_TOLERENCE <- 1.0;
	...
	//paramerter for choosing equation
	...
	/*************************************************
	 * 				LANDUSE'S PARAMETERS
	 *************************************************/
	float MAX_WORLDTEMPC <- 50.0;
	float AVG_EGG_BSSURFACE <- 50.0; // adapted from Wong et al., 2011
	float MIN_TEMP_USECOOLER <- 30.0;// can be changed depending on study area!
	...
	/***************************************************************************************************
	 * Reflex functions is exected for each step depending on the providing conditions of each function
	 ***************************************************************************************************/
	...
	reflex updateWeather when: bNewDay
	{
		fGlobalTodayAirTempC <- (list_daily_max_temp[nb_days] + list_daily_min_temp[nb_days])/2;
		fGlobalTodayRainfall <- list_daily_rain[nb_days];
		write "" + nb_days+ ": "+ fGlobalTodayRainfall +" & temperature: "+ fGlobalTodayAirTempC;
	} 
...
//			do layEgg; 
		...
		action killMe(string cause)
		{
			...
			if(bCheckMaxDisp)
			{
				save[bornObj, name, iAge, fMaxLifeDisps, cause] type: csv to: "dispersal.csv";
			}
			do die;
		}
...
//	     	//---------- pupae ----------------------
	...
	/************************************
	 * Use to save an activity's location
	 ************************************/ 
...
// 				EXPERIMENTS
/////////////////////////////////////////////////////////
experiment 'Run model with graphics' type: gui 
{
	output 
	{
		display showDispersal
		{
			species SpatObj aspect: showDispersal transparency: 0.2;
...
experiment 'Run 5 sims in batch mode' type: batch repeat: 5 keep_seed: true until: nb_days = sim_days_duration or length(Aedes) = 0 
{
    //change the repeat: "number" to increase or decrease the simulation times
}
